...
export default async function DashboardPage() {
  ...
  const { count: totalRiders } = await supabase
    .from("riders").select("*", { count: "exact", head: true });

  // Top movers: fetch after-snapshots, compute latest vs avg per rider
  const { data: afterSnaps } = await supabase
    .from("snapshots")
    .select("rider_id, points, race, fetched_at")
    .eq("snapshot", "after")
    .gt("points", 0)
  ...
  // Compute top movers
  type MoverSnap = { rider_id: number; points: number; race: string; fetched_at: string };
  ...
  // Fetch rider names for movers
  const moverRiders: Record<number, string> = {};
  if (topMoverIds.length > 0) {
    const { data: moverRows } = await supabase
      .from("riders").select("id, full_name, team_abbr").in("id", topMoverIds);
    for (const r of (moverRows ?? []) as { id: number; full_name: string; team_abbr: string }[]) {
  ...
  const topMovers = moverScores.slice(0, 3).map((m) => ({
    ...m,
    name: moverRiders[m.rider_id] ?? `Rytter #${m.rider_id}`,
  }));
  ...
  return (
    <div className="px-8 py-9 max-w-3xl space-y-8">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Ryttere", value: totalRiders ?? 0, sub: "i databasen" },
          { label: "Løb med data", value: completedCount, sub: `af ${TOTAL_RACES}` },
          { label: "Fremgang", value: `${progressPct}%`, sub: "af sæsonen" },
        ].map(({ label, value, sub }) => (
          <div
            key={label}
          ...
          >
            ...
            <p className="text-3xl font-bold mt-1.5 tabular-nums" style={{ color: "var(--c-text)" }}>{value}</p>
      ...
      {nextRace ? (
        <div
          ...
          style={{ backgroundColor: "var(--c-surface)", border: "1px solid rgba(59,130,246,0.2)" }}
        >
          <div className="flex items-center gap-2 mb-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ color: "var(--c-blue)" }}>
              Næste løb
            </p>
      ...
      <Suspense
        fallback={
          <div
            className="rounded-xl px-6 py-5 flex items-center gap-2 text-sm"
      ...
      <div className="rounded-xl px-6 py-5 space-y-4" style={{ backgroundColor: "var(--c-surface)", border: "1px solid var(--c-border)" }}>
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs" style={{ color: "var(--c-muted)" }}>
            {completedCount} / {TOTAL_RACES} afsluttet
          </p>
        </div>
        ...
        <div className="h-[3px] rounded-full overflow-hidden" style={{ backgroundColor: "var(--c-border)" }}>
          <div
            ...
            style={{ width: `${progressPct}%`, backgroundColor: "var(--c-green)" }}
          />
        </div>
          ...
          <div className="space-y-3">
            {topMovers.map((m, i) => (
              <div key={m.rider_id} className="flex items-center gap-3">
                <span className="text-[10px] tabular-nums w-4 text-right shrink-0" style={{ color: "var(--c-border)" }}>
                  {i + 1}
                </span>
